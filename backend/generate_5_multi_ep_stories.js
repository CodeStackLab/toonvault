...
const STORIES_TO_GENERATE = [
    {
        title: "The Secret Heiress & The Dragon Duke",
        genre: "Romance",
        description: "An exiled noble daughter hides her royal lineage as a maid, until the mysterious Dragon Duke recognizes her ancient magic.",
        episodePrompts: [
            "Episode 1: The Forbidden Commute. The heiress works in the ducal manor hiding her glowing crest.",
            "Episode 2: Unmasking the Rival. The Duke catches her using ancient spellcraft in the library.",
            "Episode 3: Dance of the Starlight Ball. She is forced to attend the grand ball in disguise.",
            "Episode 4: Oath of the Dragon Crown. The Duke pledges his army to help her reclaim the throne."
        ]
    },
    {
        title: "Solo Leveling: Shadow Overlord",
        genre: "Action",
        description: "Trapped inside an S-Rank double dungeon, an F-Rank hunter unlocks an ancient Monarch System that allows him to command an army of shadows.",
        episodePrompts: [
            "Episode 1: Awakening in the F-Rank Dungeon. Facing the stone statues and claiming the secret quest.",
            "Episode 2: The Necromancer's First Army. Arise! Summons his first shadow knight companion.",
            "Episode 3: Raid on the Monarch's Gate. Soloing a guild boss in front of stunned hunters.",
            "Episode 4: Rise of the Shadow Sovereign. Unlocking the true domain of the Shadow Monarch."
        ]
    },
    {
...
async function generateImageRunware(prompt, seed) {
    try {
        const runwareTasks = [
            { taskType: "authentication", apiKey: RUNWARE_KEY },
            {
                ...
                model: "runware:100@1",
                positivePrompt: `masterpiece, ultra-detailed Korean manhwa webtoon art style, official anime aesthetic, cinematic lighting, 8k resolution, ${prompt}`,
                negativePrompt: "blurry, low quality, distorted, bad anatomy, extra limbs, watermark, text, logo, nsfw",
                width: 512,
                height: 768,
                numberResults: 1,
                outputFormat: "WEBP",
        ...
        const resp = await axios.post("https://api.runware.ai/v1", runwareTasks, { timeout: 45000 });
        const imgData = resp.data?.data?.find(d => d.taskType === "imageInference");
        if (imgData && imgData.imageURL) {
            return imgData.imageURL;
        }
    } catch (err) {
        console.warn("Runware API call failed/fallback:", err.message);
    }

    // Fallback: Pollinations AI
    const cleanPrompt = `masterpiece Korean manhwa webtoon style, ${prompt}`;
...
async function generateScriptMistral(systemPrompt, userPrompt) {
    try {
        ...
        console.error("Mistral Script Error:", e.response?.data || e.message);
        throw e;
    }
}
...
async function main() {
    ...
    console.log("🚀 Starting Multi-Episode Story Generation Pipeline...\n");

    for (let sIdx = 0; sIdx < STORIES_TO_GENERATE.length; sIdx++) {
        const item = STORIES_TO_GENERATE[sIdx];
        console.log(`===========================================================`);
        console.log(`📖 Generating Story ${sIdx + 1}/${STORIES_TO_GENERATE.length}: "${item.title}" (${item.genre})`);
        console.log(`===========================================================`);
        ...
        for (let epIdx = 0; epIdx < item.episodePrompts.length; epIdx++) {
            ...
            for (let pIdx = 0; pIdx < (scriptPanels || []).length; pIdx++) {
                ...
                console.log(`     ├─ Generating Panel ${pIdx + 1} Image with Runware (seed: ${seed})...`);
                const imgUrl = await generateImageRunware(panel.imagePrompt || item.title, seed);
            ...
            episodesList.push({
                ...
                choices: (choices || [{ text: "Continue plot" }, { text: "Add twist" }]).map(c => ({ text: c.text, votes: Math.floor(Math.random() * 80) + 10 })),
                createdAt: new Date()
            });
        }
